...
class SpecificRecordDecoderTest : FunSpec({
   ...
   test("basic record decoder") {
      ...
      SpecificRecordDecoder(Foo::class).decode(schema, record) shouldBe Foo("hello", true)
   }
   ...
   test("enums") {
      ...
      SpecificRecordDecoder(Foo::class).decode(schema, record) shouldBe Foo(Wine.Shiraz)
   }
   ...
   test("nulls") {
      ...
      SpecificRecordDecoder(Foo::class).decode(schema, record) shouldBe Foo(null, "hello")
   }
   ...
   test("sets") {
      ...
      SpecificRecordDecoder(Foo::class).decode(schema, record) shouldBe Foo(
         setOf(1, 2),
         setOf(1L, null, 2L),
   ...
   test("maps") {
      ...
      SpecificRecordDecoder(Foo::class).decode(schema, record) shouldBe Foo(map)
   }
   ...
   test("maps using UTF8") {
      ...
      SpecificRecordDecoder(Foo::class).decode(schema, record) shouldBe Foo(mapOf("a" to 1, "b" to 2))
   }
   ...
   test("maps of maps") {
      ...
      SpecificRecordDecoder(Foo::class).decode(schema, record) shouldBe Foo(maps)
   }
})
